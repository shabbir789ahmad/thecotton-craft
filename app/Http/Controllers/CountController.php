...
class CountController extends Controller
{
    function count()
    {
        ...
        $products=Product::where('total','<=','3')->get();
      ...
      whereDay('orders.created_at', date('m'))->take('3')->get();
        ...
        $message=Contact::whereDay('created_at', date('d'))->latest()->take(3)->get();

        $today=Order::
        join('details','orders.id','=','details.order_id')->select('orders.id','orders.name','details.product','details.order_status','orders.phone','details.quentity')
        ->whereDay('orders.created_at', date('d'))->take(10)->get();
    ...
    return view('Dashboard.dashboard',$arr,compact('user','pr','order','or','sale','sl','earn','en','message','today','order2','products'));
    }
