...
<div class="row">
 <div class="col-lg-12">
  ...
  <tr>
     <td class="col-2"><img src="{{asset('uploads/img/' .$shop['image'])}}" width="100%" height="100rem"></td>
     <td>{{ucfirst($shop['title'])}}</td>
    <td>{{ucfirst($shop['text'])}}</td>
    <td>
     <div class="b d-flex justify-content-center mt-3">
     
                  <form  action="{{ route('shop.show',['shop' => $shop->id])}}" method="GET" class="stock_form " >
                    @csrf
                    <input type="hidden" name="brandss" class="brnd">
                   <button class="btn btn-color text-light btn-sm stc" >Update</button>
                  </form>
                  <form action="{{ route('shop.destroy', ['shop' => $shop->id]) }}" method="POST" class="ml-1" onsubmit="return confirmDelete()">
                    @method('DELETE')
                    @csrf
