...
<div class="b p-3 mt-0" style="background-color:#F0F0F0">
  ...
  <div class="c ml-3  d-flex mr-1">
   ...
   <i class="fab fa-slideshare text_color fa-lg"></i> Create
    ...
    <div class="card-body text-dark">
  <h4 class="text-center font-weight-bold text_color">Update or Delete Color</h4>
   </div>
 </div>
...
<div class="row">
 <div class="col-lg-12">
  ...
  <tr>
     ...
     <td  >
       <div style="background-color:{{$color['colors']}}; height: 5rem;"></div>
     </td>
    ...
    <td>
     <div class="b d-flex justify-content-center mt-3">
                  ...
                  <form  action="{{ route('color.show',['color' => $color->id])}}" method="GET" class="stock_form " >
                    @csrf
                    <input type="hidden" name="brandss" class="brnd">
                   <button class="btn btn-color text-light btn-xs stc" >Update</button>
                  </form>
                  <form action="{{ route('color.destroy', ['color' => $color->id]) }}" method="POST" class="ml-1" onsubmit="return confirmDelete()">
                    @method('DELETE')
                    @csrf
