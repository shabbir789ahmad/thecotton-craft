...
<p class="text-left text-md-center mt-3   ">{{ $details['currency'] }} {{ $details['price'] }}</p>
</div>
...
<p class="text-left text-md-center mt-3  ">{{ $details['currency'] }} {{ $details['price'] * $details['quantity'] }}</p>
</div>

<div class="col-md-2 col-sm-2 col-2 text-center actions " data-th="" data-id="{{ $id }}">

<button class="btn btn-danger btn-sm mt-3 remove-from-cart" data-id="{{$id}}">
  ...
  <div class="row">
    ...
    <div class="col-md-4 ">
        <div class="mt-5 g border p-5">
            @if(session('cart'))
       <p class=" mt-2 ">Total:<span class="float-right"> {{ $details['currency'] }} {{ $total }}</span></p>
       @endif
     <hr>
